interface GoogleIconProps {
    className?: string;
}

export default function GoogleIcon({ className = "" }: GoogleIconProps) {
    return (
        <svg
            width="21"
            height="20"
            viewBox="0 0 21 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className={className}
            aria-label="google icon"
        >
            <g clipPath="url(#clip0_555_18051)">
                <path
                    d="M20.3042 10.2303C20.3042 9.55056 20.2491 8.86711 20.1315 8.19836H10.6992V12.0492H16.1007C15.8765 13.2911 15.1563 14.3898 14.1018 15.0879V17.5866H17.3243C19.2166 15.8449 20.3042 13.2728 20.3042 10.2303Z"
                    fill="#4285F4"
                />
                <path
                    d="M10.7019 20.0006C13.3989 20.0006 15.6734 19.1151 17.3306 17.5865L14.1081 15.0879C13.2115 15.6979 12.0541 16.0433 10.7056 16.0433C8.09669 16.0433 5.88468 14.2832 5.091 11.9169H1.76562V14.4927C3.46322 17.8695 6.92087 20.0006 10.7019 20.0006Z"
                    fill="#34A853"
                />
                <path
                    d="M5.08467 11.9169C4.66578 10.6749 4.66578 9.33008 5.08467 8.08811V5.51233H1.76297C0.344635 8.33798 0.344635 11.667 1.76297 14.4927L5.08467 11.9169Z"
                    fill="#FBBC04"
                />
                <path
                    d="M10.7019 3.95805C12.1276 3.936 13.5055 4.47247 14.538 5.45722L17.393 2.60218C15.5852 0.904587 13.1858 -0.0287217 10.7019 0.000673888C6.92087 0.000673888 3.46322 2.13185 1.76562 5.51234L5.08732 8.08813C5.87733 5.71811 8.09302 3.95805 10.7019 3.95805Z"
                    fill="#EA4335"
                />
            </g>
            <defs>
                <clipPath id="clip0_555_18051">
                    <rect width="20" height="20" fill="white" transform="translate(0.5)" />
                </clipPath>
            </defs>
        </svg>
    );
}

